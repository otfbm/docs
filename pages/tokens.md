...
Use the built-in command to generate a shortcode for a custom token.

<!-- tabs:start -->

#### **Discord**
```
!map -tokenimport https://github.com/patpluspun/valley-of-the-infernal-sun/blob/b89c678a009f95369af41d10ca048d1edfbbffbf/portraits/gravedigger-clod.png
```

<!-- tabs:end -->

This will generate a link to your shortcode. Copy that text.

<!-- tabs:start -->

#### **Discord**

`<shortcode>` should be the shortcode generated by the -tokenimport link. `<token-name>` should be an easy to remember name, like `mike`, though it must be unique for your user, as it is saved in a uvar.

The token must be loaded on every new battle, though saved tokens persist until replaced.
```
!map -savetoken <token-name>:<shortcode>
!map -t <character> -loadtoken <token-name>
```

#### **Url**

For url editing, one must paste the shortcode directly behind the token the image is for.
```
...
**Facial Recognition**
Token facial recognition can be intentionally turned off on a case by case basis. Adding a trailing tilde `~` will skip facial recognition and crop the token to the image constraints instead.

<!-- tabs:start -->

#### **Discord**

Just add the tilde `~` to the end of the saved shortcode.
```
!map -t satyr -move a1 -token wcqj3
!map -t satyr -move b1 -token wcqj3~
```

#### **Url**

Add the tilde `~` to the end of the desired shortocde in the url.
```
https://otfbm.io/a1-satyr~wcqj3/b1-satyr~wcqj3~
```
