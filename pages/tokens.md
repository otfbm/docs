...
`<shortcode>` should be the shortcode generated by the -tokenimport link. `<token>` should be an easy to remember name, like `mike`, though it must be unique for your user, as it is saved in a uvar.

The token must be loaded on every new battle, though saved tokens persist until replaced.
```
!map -savetoken <token>:<shortcode>
!map -t <character> -loadtoken <token>
```
